...
- `membership."MembershipSetting"` - per-company SINGLETON (unique `companyId`): `clubType` (`golf` | `leisure` | `others`, fixed vocabulary), `isCommittee`, `salesAgencyEnabled` / `salesExternalEnabled` / `salesInternalEnabled` (forced false while `isCommittee` - committee clubs have no sales agents), RBAC stamps (nullable - the row may be system-created before anyone edits).
