...
- `GET /settings` · `PUT /settings` (upsert singleton + replace override lines atomically) · `GET /settings/membership-types` (the picker via membershipGateway - a golf admin needs no Membership menu grants) - Golf Specification, behind `requireMenuAction('/golf/settings')`.
