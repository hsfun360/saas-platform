...
- `golf.TransactionTypePackageItem` - the element lines of a PACKAGE transaction type (chargeType `package`, user decisions 2026-08-27), e.g. Weekday Twin Package 200.00 = 1 × share buggy (100.00) + 2 × caddy (50.00 each).
  Each line: `elementTransactionTypeId` (a peer transaction type of the SAME company - never a package itself, no nesting; an element in use cannot become a package) + `quantity` (1-99) + `unitAmount` (the per-unit allocation of the package price - the revenue breakdown at billing) + `sortOrder`; unique (package, element).
  ...
  TAX at billing: each element portion is taxed by the ELEMENT's own scheme via the breakdown - a package carries no tax scheme of its own (`taxSchemeCode` forced NULL server-side).
