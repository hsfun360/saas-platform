...
- Numbering at posting (user decision): `Ledger.docNo` is now nullable - auto-mode drafts show a `DRAFT-<id8>` reference and get the gapless number when they post, so voided drafts never burn one; manual-mode still requires the number at save (reserved by the unique index - Postgres unique treats NULLs as distinct). New posting-audit columns: `postedAt`, `postedBy`, `workflowInstanceId`.
