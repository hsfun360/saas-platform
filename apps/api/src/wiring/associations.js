...
const EInvoiceClassificationCode = require('../modules/saas/eInvoiceClassificationCode.model'); // reference table (LHDN e-Invoice)
const EInvoiceMsicCode = require('../modules/saas/eInvoiceMsicCode.model'); // reference table (LHDN e-Invoice)
const EInvoiceTaxType = require('../modules/saas/eInvoiceTaxType.model'); // reference table (LHDN e-Invoice)
...
// Golfer identity pair (2026-09-17), mirroring AR's Debtor/OtherDebtor: Golfer
// references its profile polymorphically (golferType + sourceId -> membership
// Member OR golf.OtherGolfer) - validated in the service, no FKs, so no
...
// 2026-08-25): unlimited category catalog, six slot columns on consumers'
// documents (ar.Ledger today). Option -> Category is a real intra-service FK
// declared in the model (like TaxRate -> TaxScheme); consumers reference
...
const ArTransactionType = require('../modules/ar/transactionType.model'); // AR-owned billing/receipting catalog since 2026-08-15 (companyId value ref; tax by code via seam; no associations)
