// Transaction Type master file (Membership Management → Master File Setup).
// The billing-item catalog: code + charge type + description + THE tax scheme
// (single source - consuming rows don't store their own tax).
...
// country-resolved via the tax seam) and OUTPUT-class (charges to members are
...
// GET /api/membership/transaction-types/meta - the charge-type options.
exports.getMeta = async (req, res) => {
    res.status(200).json({ chargeTypes: CHARGE_TYPES });
};

// GET /api/membership/transaction-types/tax-schemes - the company's usable
// OUTPUT tax schemes for the picker (same source as the Membership Fee screen).
...
// GET /api/membership/transaction-types - every transaction type for the company.
exports.list = async (req, res) => {
    try {
        const companyId = companyIdOf(req);
        if (!companyId) return res.status(400).json({ message: 'Select a workspace first.' });

        const rows = await TransactionType.findAll({ where: { companyId }, order: [['transactionType', 'ASC']] });
...
// POST /api/membership/transaction-types
...
// PUT /api/membership/transaction-types/:id - full update.
...
// PATCH /api/membership/transaction-types/:id - toggle isActive only.
