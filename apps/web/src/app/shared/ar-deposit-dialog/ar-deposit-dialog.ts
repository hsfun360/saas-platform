...
// The Deposit entry dialog (deposit slice 2026-09-01), shared by the Debtor
// Account screen (debtor preset) and the standalone Deposit screen. Opening a
// deposit is a BILLING act - it demands collateral; the money itself arrives
// later through an Official Receipt against the posted deposit. So the form
// carries no payment fields, just the required amount.
// Save keeps an editable Open draft; Submit routes through the ar-deposit
// approval chain when one is active, else posts (opens for collection).
@Component({
  selector: 'app-ar-deposit-dialog',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, DialogComponent, MoneyInputDirective],
  templateUrl: './ar-deposit-dialog.html',
  // Same chrome as the ledger dialog (.ald-* pick list / debtor band).
  styleUrls: ['../ar-ledger-dialog/ar-ledger-dialog.css'],
})
export class ArDepositDialogComponent implements OnInit {
  ...
  // Editing an existing Open (draft) deposit: prefill + PATCH instead of POST.
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    description: [''],
    // Multicurrency: the rate at billing on a foreign account.
    exchangeRate: ['', [Validators.pattern(AR_RATE_PATTERN)]],
  });

  // --- Multicurrency - same mechanics as the receipt/refund dialogs ---
  ...
  ngOnInit(): void {
    ...
    this.form.reset({
      docNo: '', docDate: t, trxDate: t, amount: 0,
  ...
  numberingMode(): string | null {
    return this.effMeta()?.numberingModes?.['ar-deposit'] ?? null;
  }
  ...
  private saveRequest() {
    ...
    if (editId) return this.service.updateDeposit(editId, this.payload());
    return this.service.createDeposit({ ...this.payload(), debtorId: debtor!.id });
  }
  ...
  onSave(): void {
    ...
    this.saveRequest().subscribe({
      ...
      error: (err) => {
        ...
        this.failed.emit(err.error?.message || 'Failed to save the deposit.');
      },
    });
  }
  ...
  onSubmit(): void {
    ...
    this.saveRequest().subscribe({
      next: (saved) => {
        ...
        this.service.submitDeposit(saved.id).subscribe({
          next: (res) => { this.saving.set(false); this.posted.emit(res.message); },
          error: (err) => {
            this.saving.set(false);
            this.failed.emit(`${err.error?.message || 'Submit failed.'} The deposit stays saved as Open.`);
          },
        });
      },
      error: (err) => {
