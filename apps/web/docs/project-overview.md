...
Any picker over a **long or unbounded reference list** uses the shared **`<app-combobox>`** (`src/app/shared/combobox`) instead of a native `<select>` - a native select has no substring search and degrades at hundreds of options.
**The objective is KEYBOARD-ONLY data entry** (user, 2026-09-05): a clerk keys a whole form without reaching for the mouse - Tab into the picker (it selects its text and opens the list), type 2-3 characters to filter, Enter commits (or Tab alone commits an exact/single match) and focus moves on.
Every entry-flow decision should preserve that property: dialogs focus their first field on open, Enter submits from any field, Esc backs out, and no field may require a mouse click to operate.
**Rolled out app-wide 2026-09-05 (user standard: "besides short fixed enums, others use the combobox")** - every reference-DATA picker is a combobox: countries, currencies, timezones, tax schemes, transaction types, e-Invoice classifications, companies, users, roles, departments, positions, agents/agencies, salutations/titles/nationalities/races/industry types, membership type/status/fee catalogs, unit courses, parent menus, open documents/deposits.
Native `<select>` remains ONLY for short FIXED vocabularies: status enums (active/suspended/closed), genders, marital statuses, credit flags, statement modes, dependent/address types, day/nine scopes, charge/course types, frequencies, months, priorities, numbering purposes/modes, ie flags, tax classes/types, approver types, approval modes, condition ops, class filters, billing types, and the small language switchers.
When adding a picker, ask: is the option list a CATALOG someone maintains (→ combobox) or a hardcoded vocabulary (→ native select)?
**Month pickers are comboboxes too - never `<input type="month">`** (Firefox does not implement it; it degrades to a bare text box showing the raw "YYYY-MM").
Use the shared `monthComboOptions()` helper (`shared/month-options.ts`): values stay the same "YYYY-MM" strings, labels render in the device's regional format, newest first.
(`<input type="date">` stays native - every browser implements it.)
...
Bind `[options]` (`{value, label}` - put code AND description in the label so both match) plus `[inputId]` for the `<label for>`; `allowEmpty`/`emptyLabel` control the None row (`[allowEmpty]="false"` for required fields).
It binds TWO ways: `[value]`/`(valueChange)` for signal-driven callers, or **`formControlName`** - it is a ControlValueAccessor like `<app-phone-input>` (touched wires on leave, so `showError()` works; pass `[invalid]` + `describedBy` for the aria wiring of the error line).
...
Reference implementations: the Analysis Dimension pickers (value-mode) and the Transaction type / Payment method fields (formControlName-mode), all in the shared AR entry dialogs.
...
Rows with 1-2 controls never need the tag.
Prefer the overflow menu for new screens; no screen currently composes the tag.
