...
| Worker | Cloud Run `platform-api-outboxworker` (2026-07-31): platform-api image with `--command node --args outboxworker.js`, `--min-instances 1 --no-cpu-throttling --no-allow-unauthenticated`, Cloud SQL connector, env `EMAIL_USER=hsfun360@gmail.com` + secrets `DATABASE_URL`/`EMAIL_PASS`/`SMTP_ENCRYPTION_KEY`. Verified end-to-end (forgot-password -> `password.reset` sent). NOTE: `SMTP_ENCRYPTION_KEY` was ALIGNED to the old environment's value because the copied `CompanySmtpConfigs` rows are encrypted with it - a fresh key cannot decrypt them |
